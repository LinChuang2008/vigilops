...
这篇文章不讲营销话术。我们会分享在 NightMend 中使用 AI（DeepSeek LLM）做告警降噪的实际做法、遇到的坑，以及什么有效、什么没效。
...
## AI 降噪在 NightMend 中怎么工作

我们在 NightMend 中集成了 DeepSeek（大语言模型）来做告警分析。这不是一个独立的"AIOps 模块"，而是嵌入在告警处理流程中的一个分析步骤。
...
或者直接看在线 Demo：[https://demo.lchuangnet.com](https://demo.lchuangnet.com)（`demo@nightmend.io` / `demo123`）

在告警详情页，你可以看到 AI 分析结果字段，包括根因推断、严重程度评估和建议操作。

## 几个实用建议

不管你是否用 NightMend，以下是我们在实践中总结的告警降噪建议：
...
2. **智能层**：AI 上下文分析 + 根因关联（NightMend 在做的）
3. **自动化层**：对已知问题自动修复（Runbook 引擎）
4. **治理层**：定期回顾 + 指标驱动的优化（需要团队文化）

NightMend 在第 2 层和第 3 层做了尝试。效果还在持续迭代中，我们不会声称"减少 90% 噪音"——这取决于你的具体环境和告警规则质量。
