...
1. **准备配置**：复制项目中的 `agent/agent.example.yaml` 并修改服务器地址和 Token
2. **运行 Agent**：使用 Python 运行 `python -m vigilops_agent` 或配置为 systemd 服务
3. **获取 Token**：在 VigilOps 后台 **设置 → Agent Token 管理** 中创建

Agent 运行后每 30 秒上报一次指标数据，资源占用约 20MB 内存。

> 💡 **详细配置** 请参考项目 `agent/` 目录下的示例文件
