...
## NightMend 的 MCP Server

NightMend 内置了一个 MCP Server，提供 5 个工具：
...
**但要注意：** 自动执行操作（比如重启服务）需要审批。NightMend 的 Runbook 引擎有安全检查机制，不是 AI 说执行就执行。你可以配置哪些操作需要人工审批，哪些可以全自动。

## 一个具体的使用场景

假设你用 Claude Desktop（或任何支持 MCP 的 AI 客户端）连接了 NightMend 的 MCP Server。
...
### 在 NightMend 中启用 MCP Server

NightMend 的 MCP Server 随后端一起部署，默认可用。你可以在任何支持 MCP 协议的客户端中配置：
...
访问 [https://demo.lchuangnet.com](https://demo.lchuangnet.com)（`demo@nightmend.io` / `demo123`），在界面上可以看到 MCP 相关的配置和工具列表。
...
- **运维领域**的 MCP Server 还很少——这也是 NightMend 做 MCP 集成的原因之一

对运维工具来说，支持 MCP 意味着：你的监控数据和操作能力可以被任何 AI Agent 使用。这比自己做一个 AI 功能更有想象空间——因为 AI Agent 的能力是在不断进化的，而你的工具只需要提供标准化的接口。

## 写在最后

MCP 协议还很年轻，NightMend 的 MCP 实现也还在早期。但方向很清晰：**运维工具需要从"给人看的面板"进化为"给 AI 用的接口"**。

这不意味着运维人员会被取代。相反，当 AI Agent 可以处理 80% 的日常琐事时，运维人员可以把精力放在架构优化、容量规划、故障预防这些更有价值的工作上。

如果你对 MCP + 运维的方向感兴趣，欢迎在 [GitHub Discussions](https://github.com/LinChuang2008/nightmend/discussions) 和我们交流。我们特别想知道：你希望 AI Agent 能帮你做哪些运维工作？
