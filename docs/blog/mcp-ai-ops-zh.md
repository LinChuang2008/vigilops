...
更远的未来，当 AI Agent 的可靠性足够高、Runbook 覆盖足够全的时候，理论上可以实现更大程度的自主运维。但坦率说，这还很远。ServiceNow 2025 年的报告指出，目前不到 1% 的企业达到了真正的自主修复水平。
...
访问 [http://139.196.210.68:3001](http://139.196.210.68:3001)（`demo@vigilops.io` / `demo123`），在界面上可以看到 MCP 相关的配置和工具列表。
