...
from nightmend_agent.collector import collect_system_info, collect_metrics, collect_agent_process_metrics
from nightmend_agent.checker import run_check
from nightmend_agent.config import AgentConfig, ServiceCheckConfig, DatabaseMonitorConfig
        ...
        try:
            ...
            if system == "Windows":
                ...
                if svc_check.returncode == 0 and "RUNNING" in svc_check.stdout:
                    ...
                    subprocess.Popen([sys.executable, "-m", "nightmend_agent.cli", "run"],
...
sc.exe stop NightMendAgent | Out-Null
$stopped = $false
for ($i = 0; $i -lt 30; $i++) {
    Start-Sleep -Seconds 1
    $status = sc.exe query NightMendAgent | Out-String
...
sc.exe start NightMendAgent | Out-Null
"""
        creation_flags = 0
        ...
        while True:
            try:
                ...
                if self.config.discovery.docker:
                    from nightmend_agent.discovery import discover_docker_services
                ...
                if self.config.discovery.host_services:
                    from nightmend_agent.discovery import discover_host_services
    ...
    async def _discovery_loop(self):
        ...
        while True:
            ...
            try:
                ...
                if self.config.discovery.docker:
                    from nightmend_agent.discovery import discover_docker_services, discover_stopped_docker_services
    ...
    async def start(self):
        ...
        if self.config.discovery.docker:
            try:
                from nightmend_agent.discovery import discover_docker_databases
        ...
        if self.config.discovery.docker:
            from nightmend_agent.discovery import discover_docker_log_sources
        ...
        # 日志采集
        if log_sources:
            from nightmend_agent.log_collector import LogCollector
